...
/// <summary>Returns per-ISO-week DPM aggregates for the past 3 weeks (including current partial week).</summary>
public sealed record GetFniDpmWeeklyQuery(
    string  Customer,
    string? Family) : IRequest<Result<IReadOnlyList<FniDpmWeeklyDto>>>;

public sealed class GetFniDpmWeeklyQueryHandler
    : IRequestHandler<GetFniDpmWeeklyQuery, Result<IReadOnlyList<FniDpmWeeklyDto>>>
{
    private readonly IPostgresqlDbContext          _db;
    private readonly IOptionsMonitor<DpmSettings>  _dpmOptions;

    public GetFniDpmWeeklyQueryHandler(
    ...
    public async Task<Result<IReadOnlyList<FniDpmWeeklyDto>>> Handle(
        GetFniDpmWeeklyQuery request, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(request.Customer))
            return Result.Failure<IReadOnlyList<FniDpmWeeklyDto>>(
        ...
        var currentWeekStart = DateTime.SpecifyKind(GetWeekStart(DateTime.Today),             DateTimeKind.Unspecified);
        var since            = DateTime.SpecifyKind(currentWeekStart.AddDays(-14),            DateTimeKind.Unspecified);
        ...
        if (!allFamilies)
        {
            ...
            if (familyIds.Count == 0)
                return Result.Success<IReadOnlyList<FniDpmWeeklyDto>>(Array.Empty<FniDpmWeeklyDto>());
        }
        ...
        var result = rows
            .GroupBy(r => GetWeekStart(r.EndTime!.Value.Date))
            .Select(g =>
            {
                var total = g.Count();
                var fail  = g.Count(r => !IsPass(r.Status));
                return new FniDpmWeeklyDto(
        ...
        return Result.Success<IReadOnlyList<FniDpmWeeklyDto>>(result);
    }
